 ...
 *        devolve um link de download que vale 2 minutos.
...
const URL_DRIVE = Deno.env.get('DRIVE_WEBAPP_URL') ?? '';
const SEGREDO = Deno.env.get('DRIVE_HMAC_SEGREDO') ?? '';
...
let chaveHmac: Promise<CryptoKey> | null = null;
const chave = () =>
  (chaveHmac ??= crypto.subtle.importKey('raw', codificador.encode(SEGREDO), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']));

async function assinar(texto: string): Promise<string> {
  const assinatura = new Uint8Array(await crypto.subtle.sign('HMAC', await chave(), codificador.encode(texto)));
...
async function chamarDrive(acao: string, dados: Record<string, unknown>): Promise<Record<string, unknown>> {
  if (!URL_DRIVE || !SEGREDO) throw new DriveIndisponivel('Drive não configurado');
  ...
  try {
    ...
    r = await fetch(URL_DRIVE, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ corpo, assinatura: await assinar(corpo) }),
      redirect: 'follow',
      signal: AbortSignal.timeout(60000), // Apps Script travado não prende o pedido
    });
  } catch (e) {
...
async function descartar(arquivoId: string, driveId: string | null): Promise<void> {
  const { error } = await admin.from('arquivos_entrega').update({ descartado_em: new Date().toISOString() }).eq('id', arquivoId);
  if (error) console.error('[entregas-drive] limpeza: não marcou o descarte', { arquivoId, codigo: error.code });
  if (driveId) await mandarParaLixeira(driveId);
}
...
function conteudoBateComTipo(b: Uint8Array, mime: string): boolean {
  ...
  switch (mime) {
    case 'application/pdf': return comeca(0x25, 0x50, 0x44, 0x46, 0x2d); // %PDF-
    case 'image/png': return comeca(0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a);
    case 'image/jpeg': return comeca(0xff, 0xd8, 0xff);
    case 'image/webp': return comeca(0x52, 0x49, 0x46, 0x46) && b[8] === 0x57 && b[9] === 0x45 && b[10] === 0x42 && b[11] === 0x50;
    case 'text/plain':
...
async function mandarParaLixeira(driveId: string): Promise<void> {
  try {
    const r = await chamarDrive('lixeira', { id: driveId });
...
async function enviar(req: Request): Promise<Response> {
  ...
  // Pedido grande demais é recusado antes de ser lido (10 MB + margem do formulário)
  const tamanhoDoPedido = Number(req.headers.get('Content-Length') ?? '0');
  if (!tamanhoDoPedido || tamanhoDoPedido > TAMANHO_MAXIMO + 1024 * 1024) {
  ...
  if (arquivo.size < 1 || arquivo.size > TAMANHO_MAXIMO) return resposta(400, { erro: 'O arquivo precisa ter até 10 MB.' });
  ...
  if (comentario.length > 10000) return resposta(400, { erro: 'O comentário passa de 10.000 caracteres.' });
  if (link.length > 2000) return resposta(400, { erro: 'O link é longo demais.' });
  ...
  if (!conteudoBateComTipo(bytes, arquivo.type)) {
    return resposta(400, { erro: 'O conteúdo do arquivo não bate com o tipo. Envie o arquivo original (PDF, imagem, ZIP, TXT ou Office).' });
  }
  const nome = nomeParaDownload(arquivo.name, ext);
  ...
  const { data: atividade } = await usuario
    .from('atividades').select('turma_id, exige_texto, exige_link, tipo_link').eq('id', atividadeId).maybeSingle();
  if (!atividade) return resposta(404, { erro: 'Atividade não encontrada.' });
  if (atividade.exige_texto && !comentario) return resposta(409, { erro: 'Esta atividade pede um comentário ou resposta.' });
  if (atividade.exige_link && !link) {
    return resposta(409, {
      erro: atividade.tipo_link === 'github' ? 'Esta atividade pede o link do GitHub.'
        : atividade.tipo_link === 'drive' ? 'Esta atividade pede o link do Google Drive.'
  ...
  const { data: reservas, error: erroReserva } = await usuario.rpc('reservar_arquivo', {
    p_atividade: atividadeId, p_nome: nome, p_mime: arquivo.type, p_tamanho: arquivo.size,
  });
  ...
  let driveId: string;
  try {
    const r = await chamarDrive('enviar', {
      turma: atividade.turma_id,
  ...
  const { error: erroRegistro } = await admin.from('arquivos_entrega').update({ drive_id: driveId }).eq('id', reserva.arquivo_id);
  if (erroRegistro) {
...
async function gerarLink(req: Request): Promise<Response> {
  ...
  if (!SEGREDO) return resposta(503, { erro: 'Download indisponível agora.' });

  const ticket = `${id}.${Math.floor(Date.now() / 1000) + VALIDADE_LINK_S}`;
  const t = `${ticket}.${await assinar(`baixar:${ticket}`)}`;
...
async function baixar(url: URL): Promise<Response> {
  ...
  const pagina = (status: number, texto: string) =>
    new Response(texto, { status, headers: { ...CABECALHOS_CORS, 'Content-Type': 'text/plain; charset=utf-8', 'Cache-Control': 'no-store' } });
  if (!id || !expira || !assinatura || !SEGREDO) return pagina(400, 'Link inválido.');
  if (!iguais(await assinar(`baixar:${id}.${expira}`), assinatura)) return pagina(403, 'Link inválido.');
  if (Number(expira) < Date.now() / 1000) return pagina(410, 'Este link expirou. Volte ao portal e clique no arquivo de novo.');

  const { data: arquivo } = await admin
    .from('arquivos_entrega').select('drive_id, nome, mime').eq('id', id)
    .is('descartado_em', null).not('drive_id', 'is', null).maybeSingle();
  if (!arquivo) return pagina(404, 'Arquivo não encontrado.');

  try {
    const r = await chamarDrive('baixar', { id: arquivo.drive_id });
    ...
    return new Response(bytes, {
      headers: {
        ...CABECALHOS_CORS, // o portal baixa com fetch (e mostra o erro no botão, se houver)
        ...
        'Content-Disposition': `attachment; filename="entrega.${TIPOS[arquivo.mime] ?? 'bin'}"; filename*=UTF-8''${
          encodeURIComponent(arquivo.nome).replace(/['()*]/g, (c) => `%${c.charCodeAt(0).toString(16).toUpperCase()}`)
        }`,
