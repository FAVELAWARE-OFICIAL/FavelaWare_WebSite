...
import {
  enviarTentativa, formatarDataHora, formatosEmTexto, podeEnviar, ROTULO_SITUACAO, ROTULO_TIPO_LINK, situacaoDoAluno,
  tiposAceitos, type Formato,
  TAMANHO_MAXIMO_ARQUIVO, tentativasDe, validarEntrega, type Atividade, type EtapaEnvio, type Situacao,
} from '../../lib/atividades';
...
const JanelaDaAtividade: React.FC<PropsJanela> = ({ atividade, participanteId, demonstracao, aoEnviar, onFechar }) => {
  ...
  const fechar = useCallback(() => { if (!ocupadoRef.current) onFechar(); }, [onFechar]);
  ...
  useEffect(() => {
    if (mensagem?.tipo === 'erro' || avisoDemonstracao) {
      ...
      document.getElementById(avisoDemonstracao ? 'aviso-demonstracao' : 'aviso-entrega')
  ...
  const enviar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLink(''); setComentario(''); setArquivo(null);
  ...
  const dicaDoLink =
    atividade.tipo_link === 'github' ? 'https://github.com/seu-usuario/seu-repositorio'
    : atividade.tipo_link === 'drive' ? 'https://drive.google.com/…'
  ...
  const topo = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <span className={`ml-auto flex items-center gap-1.5 ${texto.apoio}`}>
        ...
        {situacao === 'pendente' && <span className="font-semibold text-gray-700">· {quantoFalta(atividade.prazo)}</span>}
      </span>
    </div>
  );
  ...
  const rodape = envioAberto ? (
    <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
      <p className={texto.apoio}>
        {temRegras ? 'Os campos marcados com * são obrigatórios.' : 'Preencha pelo menos um: link, comentário ou arquivo.'}
      </p>
      <Botao type="submit" form="form-entrega" variante="primario" disabled={ocupado} className="sm:min-w-[11rem]">
        {ocupado && <Girando />}
        {etapa === 'arquivo' ? 'Enviando para o Drive…' : etapa === 'registro' ? 'Registrando…' : numero > 1 ? `Enviar ${numero}ª tentativa` : 'Enviar entrega'}
      </Botao>
    </div>
  ) : undefined;
    ...
    <Janela titulo={atividade.titulo} aberta onFechar={fechar} larga topo={topo} rodape={rodape} focoInicial="fechar">
      <div className="space-y-6">
        ...
        <section aria-labelledby="titulo-enunciado">
          <h3 id="titulo-enunciado" className={`mb-2 ${texto.rotuloMaiusculo}`}>O que fazer</h3>
        ...
        {temRegras && envioAberto && (
          <section aria-labelledby="titulo-exigencias">
            <h3 id="titulo-exigencias" className={`mb-2 ${texto.rotuloMaiusculo}`}>O que você precisa enviar</h3>
            <ul className="grid gap-2 sm:grid-cols-3">
              {atividade.exige_texto && <Exigencia titulo="Comentário ou resposta" detalhe="Escreva no campo de texto" />}
              {atividade.exige_link && <Exigencia titulo={ROTULO_TIPO_LINK[atividade.tipo_link]} detalhe={dicaDoLink} />}
              {atividade.exige_arquivo && <Exigencia titulo="Arquivo" detalhe={`${formatosEmTexto(atividade.formatos)}, até 10 MB`} />}
            </ul>
          </section>
        )}
        ...
        {situacao === 'encerrada' && (
          <div role="status" className={`flex gap-3 rounded-lg border p-4 text-sm ${estado.atencao}`}>
            ...
            <div>
              ...
              <p className="mt-1">O envio das tarefas não está mais disponível. Se tiver alguma dúvida, fale com o seu instrutor.</p>
            </div>
          </div>
        )}
        ...
        {tentativas.length > 0 && (
          <section aria-labelledby="titulo-historico" className="border-t border-gray-100 pt-5">
            <h3 id="titulo-historico" ref={tituloHistorico} tabIndex={-1}
        ...
        {envioAberto && (
          <form id="form-entrega" onSubmit={enviar} aria-labelledby="titulo-entrega" className="space-y-4 border-t border-gray-100 pt-5">
            <div>
              <h3 id="titulo-entrega" className={texto.titulo}>{numero > 1 ? `Sua ${numero}ª tentativa` : 'Sua entrega'}</h3>
              <p className={`mt-0.5 ${texto.apoio}`}>Depois de enviar, o instrutor corrige e a resposta aparece aqui.</p>
            </div>

            {/* Link */}
            <div>
              <label htmlFor="entrega-link" className={classeRotulo}>
                {atividade.tipo_link === 'qualquer' ? 'Link do trabalho' : ROTULO_TIPO_LINK[atividade.tipo_link]} <Obrigatorio sim={atividade.exige_link} />
              </label>
              <div className="relative">
                <IconeLink className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input id="entrega-link" type="url" inputMode="url" placeholder={dicaDoLink}
                  value={link} maxLength={2000} onChange={(e) => setLink(e.target.value)} disabled={ocupado}
            ...
            <div>
              <div className="flex items-baseline justify-between">
                <label htmlFor="entrega-texto" className={classeRotulo}>Comentário ou resposta <Obrigatorio sim={atividade.exige_texto} /></label>
              ...
              <textarea id="entrega-texto" rows={4} value={comentario} maxLength={LIMITE_COMENTARIO}
                aria-required={atividade.exige_texto}
                placeholder="Conte o que você fez, ou escreva a sua resposta aqui."
                onChange={(e) => setComentario(e.target.value)} disabled={ocupado} className={`${classeCampo} resize-y`} />
            </div>

            {/* Arquivo */}
            <CampoDeArquivo arquivo={arquivo} aoEscolher={setArquivo} desabilitado={ocupado}
              obrigatorio={atividade.exige_arquivo} formatos={atividade.formatos} />

            <div id="aviso-entrega"><Aviso mensagem={mensagem} className="" /></div>
            {avisoDemonstracao && (
              <p id="aviso-demonstracao" role="status" className={`rounded-lg border p-3 text-sm ${estado.atencao}`}>
                <span className="font-semibold">Conta de demonstração:</span> a entrega está certinha, mas não foi gravada.
                Para um aluno, ela iria para o instrutor corrigir.
              </p>
            )}
          </form>
        )}
...
const CampoDeArquivo: React.FC<{
  arquivo: File | null; aoEscolher: (f: File | null) => void; desabilitado: boolean; obrigatorio: boolean; formatos: Formato[];
  ...
  return (
    <div>
      <span className={classeRotulo} id="rotulo-arquivo">Arquivo <Obrigatorio sim={obrigatorio} /></span>
      {arquivo ? (
        <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white p-3">
          ...
          <Botao tamanho="pequeno" variante="perigo" disabled={desabilitado} aria-label={`Remover ${arquivo.name}`}
            onClick={() => { aoEscolher(null); setRemoveu(true); }}>
            Remover
          </Botao>
        </div>
      ) : (
        <label
          ...
          className={`flex cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed px-4 py-6 text-center transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-favela-green-500 ${
            arrastando ? 'border-favela-green-500 bg-favela-green-50' : 'border-gray-300 bg-white hover:border-favela-green-500 hover:bg-gray-50'
          }`}
        >
          ...
          <input id="entrega-arquivo" ref={campo} type="file" accept={tiposAceitos(formatos).join(',')} disabled={desabilitado}
            aria-labelledby="rotulo-arquivo" aria-describedby="dica-arquivo" aria-required={obrigatorio} className="sr-only"
...
const Exigencia: React.FC<{ titulo: string; detalhe: string }> = ({ titulo, detalhe }) => (
  <li className="flex items-start gap-2 rounded-lg border border-favela-green-500/40 bg-favela-green-50 px-3 py-2">
    <span aria-hidden="true" className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-favela-green-500 text-[11px] font-bold text-[#2d2a5f]">✓</span>
