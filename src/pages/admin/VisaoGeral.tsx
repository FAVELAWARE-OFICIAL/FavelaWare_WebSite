...
const pontos = (diferenca: number) =>
  `${Math.abs(Math.round(diferenca * 100))} ponto${Math.abs(Math.round(diferenca * 100)) === 1 ? '' : 's'}`;
...
const BarraComMeta: React.FC<{ valor: number | null }> = ({ valor }) => (
  <div className="relative h-2 rounded-full bg-gray-100" aria-hidden="true">
    ...
    <span
      ...
      style={{ left: `${META_FREQUENCIA * 100}%` }}
      title="Meta 75%"
    />
  </div>
);
  ...
  return (
    <>
      ...
      <section aria-labelledby="titulo-resumo" className={espaco.entreBlocos}>
        <div className="mb-4">
          <h2 id="titulo-resumo" className={texto.titulo}>
            Resumo da {edicao.nome}
          </h2>
          <p className={texto.corpo}>
            ...
            {painel.periodo && (
              <>
                {' '}
                · {plural(aulas, 'aula registrada', 'aulas registradas')} de {formatarData(painel.periodo.inicio)} a{' '}
                {formatarData(painel.periodo.fim)}
              </>
            )}
        ...
        <div className={`grid grid-cols-1 ${espaco.compacto} sm:grid-cols-2 xl:grid-cols-4`}>
          <Indicador
            ...
            contexto={
              diferencaMeta === null
                ...
                : diferencaMeta >= 0
                  ? `${pontos(diferencaMeta)} acima da meta de 75%`
          ...
          >
            {emRisco > 0 && (
              <a
                href="#atencao"
                className={`text-xs font-semibold text-favela-green-700 hover:underline ${foco} rounded`}
              >
                Ver quem são ↓
              </a>
            )}
          ...
          <Indicador
            ...
            contexto={
              <>
                Faltaram as <strong>{FALTAS_SEGUIDAS_ALERTA} últimas aulas</strong> seguidas (ou mais)
              </>
            }
          />
        ...
        rodape={
          ...
          painel.piorAula && (
            <p className={texto.apoio}>
              Melhor aula:{' '}
              <strong className="text-favela-green-700">
                {formatarData(painel.melhorAula.data)} ({formatarPercentual(painel.melhorAula.media)})
              </strong>
              {' · '}
              Pior aula:{' '}
              <strong className="text-red-600">
                {formatarData(painel.piorAula.data)} ({formatarPercentual(painel.piorAula.media)})
              </strong>
            </p>
          )
        }
      ...
      <div className={`${espaco.entreBlocos} grid grid-cols-1 ${espaco.grade} xl:grid-cols-2`}>
        ...
        >
          {totalAlunos ? (
            <GraficoBarrasPercentual
              larguraRotulo={96}
              barras={painel.porTurma.map((t) => ({
                rotulo: t.rotulo,
                valor: t.valor,
                detalhe: plural(t.alunos, 'aluno', 'alunos'),
              }))}
            />
          ) : (
            <SemDados />
      ...
      <Cartao
        ...
        acoes={
          emRisco > 0 && (
            <Botao tamanho="pequeno" onClick={verAlunosEmRisco}>
                ...
                <li key={a.id} className="flex flex-wrap items-center gap-3 py-2.5">
                  ...
                  <div className="flex w-36 items-center gap-2">
                    <div className="flex-1">
                      <BarraComMeta valor={a.frequencia} />
                    </div>
                    <span className="w-10 text-right text-sm font-semibold tabular-nums">
                      {formatarPercentual(a.frequencia)}
                    </span>
                  </div>
                </li>
              ))}
